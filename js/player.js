FallingJim = window.FallingJim || {}; ( function(FallingJim) {"use strict";
		...
		FallingJim.Player = (function() {
			...
			function player (imagerepo, positions)
			{
			...
			player.prototype = {
				...
				getSprite : function () {
					...
					switch(this.dir)
					{
						case Direction.LEFT: image =this.repo.getImage("playerLeft"); break;
						default: image= this.repo.getImage("playerRight"); 
					}
				...
				tryMoveRight: function ()
				{
					...
					{
						this.positiontransition = new Kit.Transition(function(offset){ this.x = this.x +offset;}.bind(this),
						this.movementspeed,
						function() { return (this.x >= this.position().x);}.bind(this));
